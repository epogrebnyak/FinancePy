...
@njit(fastmath=True, cache=True)
def americanBondOption_Tree_Fast(
    texp,
    strikePrice,
    faceAmount,
    couponTimes,
    ...
    for k in range(-nm, nm + 1):
        ...
        for i in range(0, numCoupons):
            ...
            if tflow >= texp:
                ...
                zcb = P_Fast(texp, tflow, rt, dt, ptexp, ptdelta, ptflow, _sigma, _a)
                cpn = couponAmounts[i]
...
@njit(fastmath=True, cache=True)
def bermudanSwaption_Tree_Fast(
    texp,
    tmat,
    strikePrice,
    faceAmount,
...
@njit(fastmath=True, cache=True)
def callablePuttableBond_Tree_Fast(
    couponTimes,
    couponFlows,
    callTimes,
    callPrices,
    ...
    return {
        "bondwithoption": callPutBondValues[0, jmax],
...
def fwdFullBondPrice(rt, *args):
    ...
    for i in range(1, numFlows):
        ...
        if tcpn >= texp:  # CHECK IF IT SHOULD BE >=
            ...
            zcb = P_Fast(
                texp, tcpn, rt, dt, ptexp, ptdelta, ptcpn, self._sigma, self._a
            )
            pv = pv + zcb * cpn
...
class FinModelRatesHW:
    ...
    def optionOnZCB(self, texp, tmat, strike, faceAmount, dfTimes, dfValues):
    ...
    def europeanBondOptionJamshidian(
        self, texp, strikePrice, face, cpnTimes, cpnAmounts, dfTimes, dfValues
    ):
        ...
        for i in range(0, numCoupons):
            ...
            if tcpn >= texp:  # coupons on the expiry date are included
                ...
                strike = P_Fast(
                    texp, tcpn, rstar, dt, ptexp, ptdelta, ptcpn, self._sigma, self._a
                )

                v = self.optionOnZCB(texp, tcpn, strike, 1.0, dfTimes, dfValues)
    ...
    def europeanBondOptionExpiryOnly(
        self, texp, strikePrice, faceAmount, cpnTimes, cpnAmounts
    ):
        ...
        for k in range(0, numNodes):
            ...
            for i in range(0, numCoupons):
                ...
                if tcpn >= texp:
                    ...
                    ptcpn = _uinterpolate(tcpn, self._dfTimes, self._dfs, interp)
    ...
    def optionOnZeroCouponBond_Tree(self, texp, tmat, strikePrice, faceAmount):
        ...
        for k in range(0, numNodes):
            ...
            zcb = P_Fast(
                texp, tmat, rt, dt, ptexp, ptdelta, ptmat, self._sigma, self._a
            )
    ...
    def bermudanSwaption(
        self, texp, tmat, strike, face, couponTimes, couponFlows, exerciseType
    ):
        ...
        payValue, recValue = bermudanSwaption_Tree_Fast(
            texp,
            tmat,
            strike,
            face,
    ...
    def bondOption(
        self, texp, strikePrice, faceAmount, couponTimes, couponFlows, exerciseType
    ):
        ...
        if exerciseTypeInt == 1:
            ...
            if self._europeanCalcType == FinHWEuropeanCalcType.JAMSHIDIAN:
                ...
                v = self.europeanBondOptionJamshidian(
                    texp,
                    strikePrice,
                    faceAmount,
                    couponTimes,
                ...
                v = self.europeanBondOptionExpiryOnly(
                    texp, strikePrice, faceAmount, couponTimes, couponFlows
                )
                ...
                callValue, putValue = americanBondOption_Tree_Fast(
                    texp,
                    strikePrice,
                    faceAmount,
                    couponTimes,
    ...
    def callablePuttableBond_Tree(
        ...
        v = callablePuttableBond_Tree_Fast(
            couponTimes,
            couponFlows,
            callTimes,
            callPrices,
    ...
    def buildTree(self, treeMat, dfTimes, dfValues):
        ...
        self._Q, self._pu, self._pm, self._pd, self._rt, self._dt = buildTree_Fast(
            self._a, self._sigma, treeTimes, self._numTimeSteps, dfTree
        )
