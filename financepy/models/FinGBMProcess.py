...
def getPaths(numPaths, numTimeSteps, t, mu, stockPrice, volatility, seed):
    ...
    for ip in range(0, numPaths):
        for it in range(1, numTimeSteps + 1):
            for ia in range(0, numAssets):
                ...
                Sall[ip + numPaths, it, ia] = Sall[ip + numPaths, it - 1, ia] * v / w
...
@njit
def getAssets(numAssets, numPaths, t, mus, stockPrices, volatilities, corrMatrix, seed):
...
class FinGBMProcess:
    def getPaths(
        ...
        paths = getPaths(numPaths, numTimeSteps, t, mu, stockPrice, volatility, seed)
    ...
    def getPathsAssets(
        ...
        if numTimeSteps == 2:
            paths = getAssets(
                numAssets, numPaths, t, mus, stockPrices, volatilities, corrMatrix, seed
            )
        else:
            paths = getPathsAssets(
                numAssets,
                numPaths,
                numTimeSteps,
                t,
