    ...
    for iPath in range(0, numPaths):
        ...
        for j in range(1, numForwards):  # TIME LOOP
            ...
            for i in range(j, numForwards):  # FORWARDS LOOP
                ...
                fwdB[i] = fwd[iPath, j - 1, i] * np.exp(
                    muA * dt - 0.5 * (zi ** 2) * dt + zi * w * sqrtdt
                )
...
def LMMSimulateFwds1F(
    numForwards, numPaths, numeraireIndex, fwd0, gammas, taus, useSobol, seed
):
