...
def objFAST(params, *args):
    ...
    K_25D_C = solveForSmileStrikeFAST(
        s,
        t,
        rd,
        rf,
    ...
    K_25D_P = solveForSmileStrikeFAST(
        s,
        t,
        rd,
        rf,
...
@njit(float64(int64, float64[:], float64, float64, float64), cache=True, fastmath=True)
...
def solveForSmileStrikeFAST(
    ...
    K = newton_secant(deltaFit, x0=initialGuess, args=argtuple, tol=1e-8, maxiter=50)
...
def solveForStrike(
    ...
    if deltaMethodValue == FinFXDeltaMethod.SPOT_DELTA.value:
        ...
        K = newton_secant(g, x0=spotFXRate, args=argtuple, tol=1e-7, maxiter=50)
...
class FinFXVolSurface:
    ...
    def volatility(self, K, expiryDate):
        ...
        if numCurves == 1:
            ...
            fwd = self._F0T[0]
            texp = self._texp[0]
            vol = volFunction(volTypeValue, self._parameters[0], fwd, K, texp)
            return vol
        ...
        if t > self._texp[-1]:
            ...
            fwd = self._F0T[-1]
            texp = self._texp[-1]
            vol = volFunction(volTypeValue, self._parameters[-1], fwd, K, texp)
            return vol
        ...
        vol0 = volFunction(volTypeValue, self._parameters[index0], fwd0, K, t0)

        fwd1 = self._F0T[index1]
        t1 = self._texp[index1]
        vol1 = volFunction(volTypeValue, self._parameters[index1], fwd1, K, t1)
    ...
    def solveForSmileStrike(
        self, optionTypeValue, deltaTarget, tenorIndex, initialValue
    ):
        """Solve for the strike that sets the delta of the option equal to the
        target value of delta allowing the volatility to be a function of the
        strike."""
        ...
        argtuple = (
            ...
            inverseDeltaTarget,
            self._parameters[tenorIndex],
        )

        K = newton_secant(
            deltaFit, x0=initialValue, args=argtuple, tol=1e-5, maxiter=50
        )
    ...
    def impliedDbns(self, lowFX, highFX, numIntervals):
        ...
        for iTenor in range(0, len(self._tenors)):
            ...
            density = optionImpliedDbn(self._spotFXRate, texp, rd, rf, Ks, vols)
