...
class FinDiscountCurve:
    ...
    def _zeroToDf(
        ...
        if freqType == FinFrequencyTypes.CONTINUOUS:
            ...
            or freqType == FinFrequencyTypes.SEMI_ANNUAL
            or freqType == FinFrequencyTypes.QUARTERLY
            or freqType == FinFrequencyTypes.MONTHLY
        ):
    ...
    def swapRate(
        ...
        for maturityDt in maturityDates:
            ...
            schedule = FinSchedule(effectiveDate, maturityDt, freqType)
    ...
    def _df(self, t: (float, np.ndarray)):
        ...
        if (
            self._interpType is FinInterpTypes.FLAT_FWD_RATES
            or self._interpType is FinInterpTypes.LINEAR_ZERO_RATES
            or self._interpType is FinInterpTypes.LINEAR_FWD_RATES
        ):

            df = interpolate(t, self._times, self._dfs, self._interpType.value)
    ...
    def bump(self, bumpSize: float):
        ...
        discCurve = FinDiscountCurve(
            self._valuationDate, times, values, self._interpType
        )
    ...
    def __repr__(self):
        ...
        for i in range(0, numPoints):
            s += labelToString("%12s" % self._dfDates[i], "%12.8f" % self._dfs[i])
