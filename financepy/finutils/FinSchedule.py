    ...
    def __init__(
        ...
        if nextToLastDate is None:
            ...
            if nextToLastDate > effectiveDate and nextToLastDate < terminationDate:
                ...
                raise FinError(
                    "Next to last date must be after effective date and"
                    + " before termination date"
                )
    ...
    def _generate(self):
        ...
        if self._dateGenRuleType == FinDateGenRuleTypes.BACKWARD:
            ...
            for i in range(1, flowNum - 1):
                ...
                dt = calendar.adjust(
                    unadjustedScheduleDates[flowNum - i - 1], self._busDayAdjustType
                )
        ...
        if self._adjustTerminationDate is True:
            ...
            self._terminationDate = calendar.adjust(
                self._terminationDate, self._busDayAdjustType
            )
    ...
    def __repr__(self):
        ...
        if 1 == 0:
            ...
            if len(self._adjustedDates) > 1:
                s += "\n"
                s += labelToString("FLW", self._adjustedDates[1:], "", listFormat=True)
