...
def timesFromDates(
    dt: FinDate, valuationDate: FinDate, dayCountType: FinDayCountTypes = None
):
...
def checkVectorDifferences(x: np.ndarray, y: np.ndarray, tol: float = 1e-6):
...
def printTree(array: np.ndarray, depth: int = None):
...
def inputTime(dt: FinDate, curve):
    ...
    def check(t):
        if t < 0.0:
            raise FinError(
                "Date " + str(dt) + " is before curve date " + str(curve._curveDate)
            )
        elif t < small:
            t = small
        return t
...
@njit(fastmath=True, cache=True)
def dotproduct(xVector: np.ndarray, yVector: np.ndarray):
...
def labelToString(
    label: str, value: float, separator: str = "\n", listFormat: bool = False
):
...
def tableToString(header: str, valueTable, floatPrecision="10.7f"):
...
@njit(fastmath=True, cache=True)
def accruedTree(gridTimes: np.ndarray, gridFlows: np.ndarray, face: float):
