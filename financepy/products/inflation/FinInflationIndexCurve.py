...
class FinInflationIndexCurve:
    ...
    def __init__(
        self, indexDates: list, indexValues: (list, np.ndarray), lagInMonths: int = 3
    ):
    ...
    def indexValue(self, dt: FinDate):
        ...
        cpiFirstValue = np.interp(cpiFirstTime, self._indexTimes, self._indexValues)

        cpiSecondValue = np.interp(cpiSecondTime, self._indexTimes, self._indexValues)
    ...
    def __repr__(self):
        ...
        for i in range(0, numPoints):
            s += labelToString(
                "%12s" % self._indexDates[i], "%10.7f" % self._indexValues[i]
            )
