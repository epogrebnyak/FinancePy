...
def f(volatility, *args):
    ...
    vdf = self.value(valueDate, spotFXRate, domDiscountCurve, forDiscountCurve, model)[
        "v"
    ]
...
def fvega(volatility, *args):
    ...
    fprime = self.vega(valueDate, spotFXRate, domDiscountCurve, forDiscountCurve, model)
...
class FinFXVanillaOption:
    ...
    def value(
        ...
        if type(model) == FinModelBlackScholes or type(model) == FinModelSABR:

            if type(model) == FinModelBlackScholes:
                volatility = model._volatility
            elif type(model) == FinModelSABR:
                volatility = volFunctionSABR(
                    model.alpha, model.beta, model.rho, model.nu, F0T, K, tdel
                )
            ...
            if self._optionType == FinOptionTypes.EUROPEAN_CALL:
                ...
                vdf = bsValue(
                    S0, texp, K, rd, rf, v, FinOptionTypes.EUROPEAN_CALL.value
                )

            elif self._optionType == FinOptionTypes.EUROPEAN_PUT:

                vdf = bsValue(S0, texp, K, rd, rf, v, FinOptionTypes.EUROPEAN_PUT.value)
                ...
                vdf = crrTreeValAvg(
                    ...
                    numStepsPerYear,
                    texp,
                    FinOptionTypes.AMERICAN_CALL.value,
                    K,
                )["value"]
                ...
                vdf = crrTreeValAvg(
                    ...
                    numStepsPerYear,
                    texp,
                    FinOptionTypes.AMERICAN_PUT.value,
                    K,
                )["value"]
            else:
    ...
    def delta_bump(
        self, valueDate, spotFXRate, ccy1DiscountCurve, ccy2DiscountCurve, model
    ):
        ...
        v = self.value(
            valueDate, spotFXRate, ccy1DiscountCurve, ccy2DiscountCurve, model
        )

        vBumped = self.value(
            valueDate, spotFXRate + bump, ccy1DiscountCurve, ccy2DiscountCurve, model
        )
    ...
    def delta(self, valueDate, spotFXRate, domDiscountCurve, forDiscountCurve, model):
    ...
    def fastDelta(self, t, s, rd, rf, vol):
        """Calculation of the FX Option delta. Used in the determination of
        the volatility surface. Avoids discount curve interpolation so it
        should be slightly faster than the full calculation of delta."""
        ...
        pips_spot_delta = bsDelta(s, t, k, rd, rf, vol, self._optionType.value)
        pips_fwd_delta = pips_spot_delta * np.exp(rf * t)
    ...
    def impliedVolatility(
        self, valueDate, stockPrice, discountCurve, dividendCurve, price
    ):
        ...
        argtuple = (self, valueDate, stockPrice, discountCurve, dividendCurve, price)

        sigma = optimize.newton(
            f, x0=0.2, fprime=fvega, args=argtuple, tol=1e-6, maxiter=50, fprime2=None
        )
        return sigma
