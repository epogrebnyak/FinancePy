...
class FinFXOption(object):
    ...
    def delta(self, valueDate, stockPrice, discountCurve, dividendCurve, model):
        """Calculate the option delta (FX rate sensitivity) by adding on a
        small bump and calculating the change in the option price."""

        v = self.value(valueDate, stockPrice, discountCurve, dividendCurve, model)

        vBumped = self.value(
            valueDate, stockPrice + bump, discountCurve, dividendCurve, model
        )
    ...
    def gamma(self, valueDate, stockPrice, discountCurve, dividendCurve, model):
        """Calculate the option gamma (delta sensitivity) by adding on a
        small bump and calculating the change in the option delta."""

        v = self.delta(valueDate, stockPrice, discountCurve, dividendCurve, model)
    ...
    def theta(self, valueDate, stockPrice, discountCurve, dividendCurve, model):
        ...
        vBumped = self.value(nextDate, stockPrice, discountCurve, dividendCurve, model)
    ...
    def rho(self, valueDate, stockPrice, discountCurve, dividendCurve, model):
        ...
        v = self.value(valueDate, stockPrice, discountCurve, dividendCurve, model)
        vBumped = self.value(
            valueDate, stockPrice, discountCurve.bump(bump), dividendCurve, model
        )
