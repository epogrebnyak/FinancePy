...
class FinFXBarrierOption(FinFXOption):
    ...
    def value(self, valueDate, spotFXRate, domDiscountCurve, forDiscountCurve, model):
        ...
        if self._optionType == FinFXBarrierTypes.DOWN_AND_OUT_CALL:
            if h >= K:
                ...
                c_di = S0 * dq * pow(hOverS, 2.0 * ll) * N(y) - K * df * pow(
                    hOverS, 2.0 * ll - 2.0
                ) * N(y - sigmaRootT)
                price = c - c_di
        elif self._optionType == FinFXBarrierTypes.DOWN_AND_IN_CALL:
            ...
            if h > K:
                p_ui = -S0 * dq * pow(hOverS, 2.0 * ll) * N(-y) + K * df * pow(
                    hOverS, 2.0 * ll - 2.0
                ) * N(-y + sigmaRootT)
                price = p_ui
            else:
    ...
    def valueMC(
        ...
        if simplePut or simpleCall:
            Sall = process.getProcess(processType, t, modelParams, 1, numPaths, seed)
        ...
        Sall = process.getProcess(
            processType, t, modelParams, numTimeSteps, numPaths, seed
        )

        (numPaths, numTimeSteps) = Sall.shape

        if (
            optionType == FinFXBarrierTypes.DOWN_AND_IN_CALL
            or optionType == FinFXBarrierTypes.DOWN_AND_OUT_CALL
            or optionType == FinFXBarrierTypes.DOWN_AND_IN_PUT
            or optionType == FinFXBarrierTypes.DOWN_AND_OUT_PUT
        ):
        ...
        if (
            optionType == FinFXBarrierTypes.UP_AND_IN_CALL
            or optionType == FinFXBarrierTypes.UP_AND_OUT_CALL
            or optionType == FinFXBarrierTypes.UP_AND_IN_PUT
            or optionType == FinFXBarrierTypes.UP_AND_OUT_PUT
        ):
