...
class FinFXVarianceSwap(object):
    ...
    def fairStrikeApprox(self, valuationDate, fwdStockPrice, strikes, volatilities):
    ...
    def fairStrike(
        ...
        optionTotal = (
            2.0 * (r * tmat - (s0 * g / sstar - 1.0) - np.log(sstar / s0)) / tmat
        )
        ...
        def f(x):
            return (2.0 / tmat) * ((x - sstar) / sstar - np.log(x / sstar))
        ...
        for n in range(0, numPutOptions):
            ...
            v = opt.value(valuationDate, s0, discountCurve, dividendCurve, model)
