...
class FinFXDigitalOption:
    ...
    def value(
        ...
        if type(model) == FinModelBlackScholes:
            ...
            if (
                self._optionType == FinOptionTypes.DIGITAL_CALL
                and self._forName == self._premCurrency
            ):
                v = S0 * exp(-rf * tdel) * N(d2)
            elif (
                self._optionType == FinOptionTypes.DIGITAL_PUT
                and self._forName == self._premCurrency
            ):
                v = S0 * exp(-rf * tdel) * N(-d2)
            if (
                self._optionType == FinOptionTypes.DIGITAL_CALL
                and self._domName == self._premCurrency
            ):
                v = exp(-rd * tdel) * N(d2)
            elif (
                self._optionType == FinOptionTypes.DIGITAL_PUT
