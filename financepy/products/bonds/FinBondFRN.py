...
def _f(dm, *args):
    ...
    px = self.fullPriceFromDM(settlementDate, nextCoupon, currentIbor, futureIbor, dm)
...
class FinBondFRN(object):
    ...
    def principal(
        ...
        fullPrice = self.fullPriceFromDM(
            settlementDate, nextCoupon, currentIbor, futureIbor, dm
        )
    ...
    def dollarDuration(
        ...
        p0 = self.fullPriceFromDM(
            settlementDate, nextCoupon, currentIbor + dy, futureIbor, dm
        )

        p2 = self.fullPriceFromDM(
            settlementDate, nextCoupon, currentIbor - dy, futureIbor, dm
        )
    ...
    def dollarCreditDuration(
        ...
        p0 = self.fullPriceFromDM(
            settlementDate, nextCoupon, currentIbor, futureIbor, dm + dy
        )
    ...
    def macauleyRateDuration(
        ...
        dd = self.dollarDuration(
            settlementDate, nextCoupon, currentIbor, futureIbor, dm
        )

        fp = self.fullPriceFromDM(
            settlementDate, nextCoupon, currentIbor, futureIbor, dm
        )
    ...
    def calcAccruedInterest(self, settlementDate: FinDate, nextCoupon: float):
