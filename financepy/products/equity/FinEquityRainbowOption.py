...
class FinEquityRainbowOption(FinEquityOption):
    ...
    def _validate(self, stockPrices, dividendCurves, volatilities, betas):
        ...
        if len(dividendCurves) != self._numAssets:
            raise FinError(
                "Dividend curves must be a vector of length " + str(self._numAssets)
            )
    ...
    def valueMC(
        ...
        self._validate(stockPrices, dividendCurves, volatilities, corrMatrix)
