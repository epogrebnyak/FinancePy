...
class FinCDSIndexPortfolio:
    ...
    def intrinsicRPV01(self, valuationDate, stepInDate, maturityDate, issuerCurves):
        ...
        cdsContract = FinCDS(stepInDate, maturityDate, 0.0)

        intrinsicRPV01 = 0.0

        for m in range(0, numCredits):

            retValue = cdsContract.riskyPV01(valuationDate, issuerCurves[m])
    ...
    def intrinsicProtectionLegPV(
        self, valuationDate, stepInDate, maturityDate, issuerCurves
    ):
        ...
        cdsContract = FinCDS(stepInDate, maturityDate, 0.0, 1.0)

        for m in range(0, numCredits):

            protectionPV = cdsContract.protectionLegPV(valuationDate, issuerCurves[m])
    ...
    def intrinsicSpread(self, valuationDate, stepInDate, maturityDate, issuerCurves):
        ...
        intrinsicProtPV = self.intrinsicProtectionLegPV(
            valuationDate, stepInDate, maturityDate, issuerCurves
        )

        intrinsicRPV01 = self.intrinsicRPV01(
            valuationDate, stepInDate, maturityDate, issuerCurves
        )
    ...
    def averageSpread(self, valuationDate, stepInDate, maturityDate, issuerCurves):
    ...
    def totalSpread(self, valuationDate, stepInDate, maturityDate, issuerCurves):
    ...
    def minSpread(self, valuationDate, stepInDate, maturityDate, issuerCurves):
        ...
        if numCredits < 1:
            raise FinError(
                "Number of credits in index must be > 1 and not" + str(numCredits)
            )
    ...
    def maxSpread(self, valuationDate, stepInDate, maturityDate, issuerCurves):
    ...
    def spreadAdjustIntrinsic(
        ...
        for j in range(0, numCDSMaturityPoints):
            ...
            cdsContract = FinCDS(valuationDate, cdsMaturityDates[j], cdsCoupon)
        ...
        for iMaturity in range(0, numIndexMaturityPoints):
            ...
            while abs(alpha - 1.0) > tolerance:
                ...
                for iCredit in range(0, numCredits):
                    ...
                    adjustedIssuerCurve = FinCDSCurve(
                        valuationDate, curveCDSContracts, liborCurve, recoveryRate
                    )
                    ...
                    cleanRPV01 = cdsIndex.riskyPV01(valuationDate, adjustedIssuerCurve)[
                        "clean_rpv01"
                    ]
        ...
        for iCredit in range(0, numCredits):
            ...
            for j in range(0, numCDSMaturityPoints):
                ...
                adjustedcdsContract = FinCDS(
                    valuationDate, cdsMaturityDates[j], adjustedSpread
                )
                ...
                adjustedIssuerCurve = FinCDSCurve(
                    valuationDate, adjustedCDSContracts, liborCurve, recoveryRate
                )
    ...
    def hazardRateAdjustIntrinsic(
        ...
        for issuerCurve in issuerCurves:
            ...
            adjustedIssuerCurve = FinCDSCurve(valuationDate, [], liborCurve)
