...
def test_FinInflationBondBBG():
    ...
    bond = FinInflationBond(
        issueDate, maturityDate, coupon, freqType, accrualType, face, baseCPIValue
    )
    ...
    ytm = bond.yieldToMaturity(settlementDate, cleanPrice, FinYTMCalcType.UK_DMO)

    testCases.print("UK DMO REAL Yield To Maturity = ", ytm)

    ytm = bond.yieldToMaturity(settlementDate, cleanPrice, FinYTMCalcType.US_STREET)

    testCases.print("US STREET REAL Yield To Maturity = ", ytm)

    ytm = bond.yieldToMaturity(settlementDate, cleanPrice, FinYTMCalcType.US_TREASURY)
    ...
    inflationAccd = bond.calcInflationAccruedInterest(settlementDate, refCPIValue)

    testCases.print("Inflation Accrued = ", inflationAccd)

    lastCpnCPIValue = 244.61839

    cleanPrice = bond.flatPriceFromYieldToMaturity(
        settlementDate, ytm, lastCpnCPIValue, FinYTMCalcType.US_TREASURY
    )

    testCases.print("Flat Price from Real YTM = ", cleanPrice)

    principal = bond.inflationPrincipal(
        settlementDate, ytm, refCPIValue, FinYTMCalcType.US_TREASURY
    )
...
def test_FinInflationBondStack():
    ...
    zciisData = [
        (FinDate(31, 7, 2020), 3.1500000000137085),
    ...
    inflationZeroCurve = FinDiscountCurveZeros(
        settlementDate,
        zcDates,
        zcRates,
        FinFrequencyTypes.ANNUAL,
