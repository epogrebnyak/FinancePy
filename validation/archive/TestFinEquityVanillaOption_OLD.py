...
from financepy.products.equity.FinEquityVanillaOptionOLD import (
    FinEquityVanillaOptionOLD,
)
...
def test_FinEquityVanillaOptionFactored():
    ...
    for numPaths in numPathsList:
        ...
        valueMC = callOption.valueMC(
            valueDate, stockPrice, discountCurve, dividendYield, model, numPaths
        )
        end = time.time()
        duration = end - start
    ...
    testCases.header(
        "NUMPATHS", "CALL_VALUE_BS", "CALL_VALUE_MC", "CALL_VALUE_MC_SOBOL", "TIME"
    )
    useSobol = True
    ...
    testCases.header(
        "NUMPATHS", "PUT_VALUE_BS", "PUT_VALUE_MC", "PUT_VALUE_MC_SOBOL", "TIME"
    )

    for stockPrice in stockPrices:

        putOption = FinEquityVanillaOptionOLD(
    ...
    for stockPrice in stockPrices:
        ...
        rho = callOption.rho(valueDate, stockPrice, discountCurve, dividendYield, model)
    ...
    for stockPrice in stockPrices:
        ...
        rho = putOption.rho(valueDate, stockPrice, discountCurve, dividendYield, model)
        testCases.print(stockPrice, value, delta, vega, theta, rho)
